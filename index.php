	...
	if(($_REQUEST['loginas']==0)||($_REQUEST['loginas']==2)){
		$result = $log->login(trim($_REQUEST['uname']),trim($_REQUEST['upass']));
			  if($result==true){
				...
				$data = array('useremail' => trim($_REQUEST['uname']),
							  'password'=>md5(trim($_REQUEST['upass'])));									
				$userInfo = $shDB->selectOnMultipleCondition($data,'logon');
				if($userInfo[0]['status'] == 0){
					$loginerrorrrr = ' <p class="greenMessage" style="color:red">Currently you are inactive. Please contact with Modpix Authority to take necessary action.</p>';		
				}else{
