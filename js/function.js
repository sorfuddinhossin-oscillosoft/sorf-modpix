...
function changeProductStatus(id, sVal, sParam){
		if(id=='')return false;
		if(sParam == 'active'){
			var answer = confirm("Are you sure to active this product?");
		}else{
		...
		if (answer){
			array = {"status" : sVal};
			$.post(base_url+"ajax/update.php",
		  			{updatedata: array,tablename : 'product', id : id}, function(data){
		  				
		  				if(parseInt(data)==1){
							if(sParam == 'active'){
								alert('You have been successfull activated this product.');
							}else{
								alert('You have been successfull deleted this product.');
							}
		  					 location.href=base_url+"user/index.php?pg=productlist&status="+sParam;
		  				}
					});			
		}
}
...
function emptyPhotoFromProd(id,prodid){
	...
	$.post(base_url+"ajax/emptyphototoproduct.php",
			...
			$('.cartImgHolder[title='+id+']').html('<img title="No Image" src="'+base_url+'/images/nophoto.png" />');				
			});	
}
...
$(document).ready(function() {
	...
	$('#addProductItemTop').click(function(){
		var prodId = $('#productidselect').val();
		if(prodId==''){alert('Please select a product');return false;}
		var itemname = $('input[name=itemname]').val();
		var itemcost = $('input[name=itemcost]').val();
...
//		alert(rel);
	...
	$('#addPhotoTohSlider').click(function(){
		$('#uploadStatus').html('<img src="'+base_url+'images/ajax-loader.gif">');
		setTimeout("$('#sliderAddImgForm').submit()",1000);
	});
	
	//delete home page slider image bof
	$('.deleteRowBtn').click(function(e){
		e.preventDefault();
		var imgId=$(this).attr('rel');
		$.post(base_url+"ajax/deleteImage.php",
			{imgId: imgId}, function(data){ 
				if(parseInt(data)==1){
					location.href=base_url+"user/index.php?pg=hpss&msg=success";
				}			  				
			});	
	});
	//delete home page slider image eof
...
$('#addCollectiontoOrder').click(function(){
	...
	$('#productcat').change(function(){
	 ...
	 $('#addMoreImageToSlider').click(function(){
			var imgAppend = '<div style="float: left; margin-top: 4px;">Image: &nbsp;<input class="mandatory" type="file" name="photoFile[]" style="width: 190px; border: 1px solid #999999;" />&nbsp;Sort Order: &nbsp;<input type="text" name="sort_order[]" value="" style="width: 20px;height: 8px;"/></div>';
			$('#uploadStatus').before(imgAppend);
			// $(imgAppend).insertBefore('#item_final');
	...
	*/
	 ...
	 //check & submit form for CMS pages bof
	 $('#addCMSBtn').click(function(){
				if($('input[name="page_name"]').val()==''){alert('Please put page name'); return false;}
				var pg_name = $('input[name="page_name"]').val();
				$.post(base_url+"ajax/checkcmspage.php",
			  			{pg_name: pg_name}, function(data){	
			  					if(parseInt(data)==1){
			  						alert('There is another page of the same name that you entered. Please choose a different page name');
			  						return false;			  						
		  						}else{
		  							$('#addCMSPage').submit();		  							
		  						}		  				 
	 ...
	 //check & submit form for CMS pages eof
	
	
	//send notification to admin for visitors request to meet photographer bof
	$("#sendContactInfo").click(function(){
		var photgrapher_id 			= $('#photgrapher_id').val();
		var c_visitor_name 			= $('#c_visitor_name').val();
		var c_visitor_address		= $('#c_visitor_address').val();
		//var c_visitor_address_1	= $('#c_visitor_address_1').val();
		var c_visitor_phone			= $('#c_visitor_phone').val();
		var c_visitor_email			= $('#c_visitor_email').val();
		var c_visitor_instruction 	= $('#c_visitor_instruction').val();
		//var c_visitor_pcode		= $('#c_visitor_pcode').val();
		//var c_visitor_city		= $('#c_visitor_city').val();
		//var c_visitor_country		= $('#c_visitor_country').val();
		
		
		if(c_visitor_name == ''){
			alert('Please type your name.');
			$('#c_visitor_name').focus();
			return false;
		}
		if(c_visitor_address == ''){
			alert('Please type your address.');
			$('#c_visitor_address"]').focus();
			return false;
		}
		if(c_visitor_phone == ''){
			alert('Please type your phone number.');
			$('#c_visitor_phone"]').focus();
			return false;
		}
		
		if(c_visitor_email == ''){
			alert('Please enter a valid email');
			$('#c_visitor_email').focus();
			return false;
		}else{
			if(checkEmail(c_visitor_email)==false){
				alert('Invalid email id');
				$('#c_visitor_email').focus();
				return false;
			}
		...
		/* if(c_visitor_city == ''){
			...
			$('#c_visitor_city"]').focus();
			return false;
		}
		if(c_visitor_country == ''){
			alert('Please type your country.');
			$('#c_visitor_country"]').focus();
			return false;
		} */
		
		$.post(base_url+"ajax/contact_request.php",
				//{photgrapher_id:photgrapher_id,c_visitor_name:c_visitor_name,c_visitor_address:c_visitor_address,c_visitor_address_1:c_visitor_address_1,c_visitor_phone:c_visitor_phone,c_visitor_email:c_visitor_email,c_visitor_pcode:c_visitor_pcode,c_visitor_city:c_visitor_city,c_visitor_country:c_visitor_country}, 
				{photgrapher_id:photgrapher_id,c_visitor_name:c_visitor_name,c_visitor_address:c_visitor_address,c_visitor_instruction:c_visitor_instruction,c_visitor_phone:c_visitor_phone,c_visitor_email:c_visitor_email}, 
				   function(data){
					if(parseInt(data)==1){
						$('#photographerContacForm').slideUp('slow');	
						$('#photographerSubmitSuccess').slideDown('slow');
					 }	
					});	
	
	});
	//send notification to admin for visitors request to meet photographer eof
	
	$("#giveHover").mouseover(function(){
		$('#giveHoverNot').hide();
		$('#giveHoverYes').show();
	});
	$("#giveHover").mouseout(function(){
		$('#giveHoverYes').hide();
		$('#giveHoverNot').show();
	});
