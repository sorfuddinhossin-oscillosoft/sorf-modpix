...
?>
<h1 class="pagetitle">Album access area</h1>
   ...
   if(isset($_REQUEST['photosecureemail'])){
	   switch ($_REQUEST['loginas']){
	   		...
	   		$selectAlbumByEmail = array(		
	   			...
	   			$selectAlbumByEmail = $shDB->selectOnMultipleCondition($selectAlbumByEmail,'`album_guest`');
									...
									if($selectAlbumByEmail){
										$selectAlbum=getAccess($selectAlbumByEmail);
		...
		if($selectAlbum){ 
			...
			if($countALbum==1){
				echo '<script>location.href=base_url+"index.php?pg=photocollection&id='.$selectAlbum[0]['id'].'";</script>';
			}
		?>
			...
			<h1>You have following photo collection:</h1>
							...
							<?php foreach($selectAlbum as $selAlbum){?>		
								...
								<td align="left" style="border-bottom:dotted #CCCCCC 1px;"><a href="<?php echo $log->baseurl;?>index.php?pg=photocollection&id=<?php echo $selAlbum['id'];?>">Enter Collection</a></td>
							</tr>	
							<?php 		
