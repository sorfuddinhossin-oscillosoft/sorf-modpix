			...
			<?php if($orderProduct){?>
				<table cellpadding="3" width="100%" cellspacing="0">
					<tr>
						...
						<td align="right" valign="bottom" class="tableHeader" style="width:48%; padding-right:2%;">Change Order Status :
						<?php 
						$status = $order['status'];
						if($status=='Enlisted'){
							$enlisted = 'Selected';
						...
						?> 
						<select id="orderStatus" title="<?php echo $_REQUEST['id'];?>">
							<option <?php echo $enlisted;?> value="Enlisted">Enlisted</option>
							<option <?php echo $onProgress;?> value="On Process">On Process</option>
							<option <?php echo $delivered;?> value="Delivered">Delivered</option>
						</select>
						</td>
					</tr>
					<tr>
							...
							if($orderProduct){
								if($order['isalbum']==1){
									...
									<td align="center" class="botBorder2" style="width:22%;padding-right:15px;"><strong>Image Id</strong></td>
														...
														if($imageInProductOrder){
															...
															<tr>								
																...
																<td align="center" class="botBorder2" style="width:22%;padding-right:15px;"><big>'.$imageId[0]['image'].'</big></td>
															</tr></table>';
																echo '</li>';
															}
														}
									...
									foreach ($orderProduct as $ordProd){
														...
														//re-code bof
														$qry = 'select p.name from prod_item pi, product p where pi.prod_id=p.id AND pi.id='.$ordProd['productid'];
														$q = $shDB->qry($qry); 
														...
														<div class="leftFloat div100" id="cartProduct<?php echo $ordProd['id']?>">
														<div align="left" class="headerCartProduct">
														<!--<span class="leftFloat"><strong>cvx<?php echo $ordProd['name'];?></strong></span>-->
														<span class="leftFloat"><strong><?php echo $product_name;?> &nbsp; <span style="color: #D95700;">(<?php echo $ordProd['name'];?>)</span></strong></span>
									...
									<td align="center" class="botBorder2" style="width:22%;padding-right:15px;"><strong>Image Id</strong></td>
														...
														$imageInProductOrder = $shDB->selectOnMultipleCondition($dataSelectfromTempImg,'order_product_img');
														...
														if($imageInProductOrder){
															...
															foreach ($imageInProductOrder as $ordProductImg){
																...
																if($product['sidehole'] ==  0){
																	$res = 0;
																
																}else{
																	$res = ($pageCounter % $product['sidehole']);
																}
															if($res == 0){
																echo '<li style="border:none;width:508px;height:17px;background:#ececec;margin:0px;clear:both;margin-left:5px;padding:4px 10px;font-weight:bold" class="cartPhotoImgHolder">Page No # '.$ordProductImg['sideid'].'</li>';
															}	
						...
						<td align="center" colspan="2" style="padding-left:1%; padding-right:1%; width:98%;">
							...
							<table cellpadding="3" cellspacing="0" style="table-layout:fixed; width:100%;">
								<tr>
									...
									<td align="center" class="botBorder2" style="width:20%"><strong>Image Quantity</strong></td>						
									<td align="center" class="botBorder2" style="width:20%"><strong>Additional Leaf</strong></td>
								...
								<?php
									...
									foreach ($orderProduct as $ordProduct){
										...
										$imgTotalPrice = $imgTotal*$ordProduct["img_price"];
										$amount = $ordProduct["basicprice"]+($ordProduct["leafprice"]*$ordProduct["leafquantity"]);
									...
									<tr>
										...
										<td align="center" class="botBorder3">'.$imageDetails[0]['album_name'].'</td>
										<td align="center" class="botBorder3">'.$ordProduct["leafquantity"].'&times;<font class="currency1">($'.$ordProduct["leafprice"].')</font></td>
								...
								-->
								<tr>
									<td align="right" colspan="4" style="padding-right:15px; font-size:16px; font-weight:bold;">Total - '.$settings['currency'].' '.$order['totalamount'].'</td>
								</tr>
								';
								?>		
							...
							<?php							
								...
								<tr>
									...
									<td align="right" class="botBorder2" style="width:10%;padding-right:15px;"><strong>sAmount</strong></td>
								</tr>
								<?php
