							...
							<div>
								...
								<?php
									if(isset($_REQUEST['albumname'])){
										...
										if(($_REQUEST['ownerhiddenfiled']!='')||($_REQUEST['ownerhiddenfiled']!='')){
										$dataforinsert = array(
											'id' => '',
											'album_name' => $_REQUEST['albumName'],
											'password' => md5('123'),
								...
								*/	
									...
									'albumname' => 'empty',
									'Album_owner' => 'empty', //multi type validation seperated by | e.g - empty|email|numeric
									'Event_date' => 'empty'
									);
									
									$validate = $shForm->validate($dataforValidation);
									...
									*/
									...
									if($validate){
										...
										<form action="" method="POST" enctype="application/x-www-form-urlencoded" name="albumAddForm">
											<label>Collection Name</label><br />
											<input class="mandatory" type="text" name="albumname" value="<?php echo $_REQUEST['albumname'];?>"><br />
												<label>Owner Email</label>&nbsp;<small>(Comma seperated multiple email possible)</small><br />
												<textarea style="width:366px;" id="albumWoner" name="albumWonertext"><?php echo $_REQUEST['albumWonertext'];?></textarea><br />
											
											   <input type="hidden" name="Album_owner" id="ownerhiddenfiled" value="<?php echo $_REQUEST['Event_date'];?>">
								                <div id='suggestionsList' style="position:relative"></div>
											<label>Event Date</label><br />
											<input class="mandatory" type="text" name="Event date" id="eventDate" value="<?php echo $_REQUEST['Album owner'];?>" readonly="readonly" onfocus="javascript:call_calendar('eventDate','eventDate'); if(this.value=='yyyy-mm-dd') this.value='';" onblur="if(this.value=='') this.value='yyyy-mm-dd';">&nbsp;&nbsp;<a id="f_trigger_b1" title="Calendar" href="javascript:call_calendar('eventDate','f_trigger_b1');" class="calIcon">&nbsp;</a><br />
									...
									<input class="" type="text" name="securecode" id="securecode" value="" ><br />
											<input type="Submit" Value="Submit" name="addAlbumBtn">
									...
									<?php 	
										...
										$result = $shDB->insert($dataforinsert,'album');
										if($result!=false){
											$dir = str_replace(chr(92),chr(47),getcwd());
											$albumName = $_REQUEST['albumname'];
											$albumName = str_replace(' ','_',$albumName);
											//$imageProcessor->readDirectory($dir);
											$mkdir = $dir.'/profile/'.$_SESSION['userid'].'/'.$albumName;											
											$dircreated = $imageProcessor->makeDirectory($mkdir);
											if($dircreated==true){
												$mkdirThumbs = $dir.'/profile/'.$_SESSION['userid'].'/'.$albumName.'/thumbs';
												$dircreatedthumbs = $imageProcessor->makeDirectory($mkdirThumbs);	
												if($dircreatedthumbs==true){
								...
								<form action="" method="POST" enctype="multipart/form-data"  name="albumAddForm" id="photocollectionform">
									...
									<label>Secure Code</label><br />
									<input class="mandatory"  type="text" name="securecode" id="securecode" value="">&nbsp;<a href="javascript:void(0)" id="genCode">Check availibility</a>
									<br />
