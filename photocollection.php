...
$album = $selectAlbum;
	...
	?>
<div class="webTitleandTab" style="clear: both; height: 67px;">
<h1 class="cartAlbumHeader"><small style="float:left;"><?php echo $album['album_name'];?></small><small style="float:right"><?php echo $album['event_place'];?></small></h1>

<div id="boxtab-blue" style="clear: both;">
	<ul>
	<li style="float:right;margin-left:165px;"><a href="<?php echo $log->baseurl;?>index.php?pg=invitation">Back to List</a></li> 
	...
	<li style="float:left"><a href="#favoritelist">Favourite Lists</a></li>
...
<div id="photos" class="tabDiv" style="margin-top: 30px;">
						...
						<div class="content">
							...
							<?php } ?>
								...
								<input type="submit" name="submit" value="Create Album" style="clear:both;margin-top:20px;">&nbsp;&nbsp;&nbsp;
								<input type="hidden" name="pcId" value="<?php echo $_REQUEST['id'];?>">
								<input type="submit" name="submit" value="Add to Favourites" style="clear:both;margin-top:20px;">
								</div>
								</form>
								<?php }else{?>
						...
						<td style="width:35%; background:#EAF0F0" valign="top">						
							...
							<a class="checkall" id="checkallconfirmed">Check All</a><a class="checkall" id="notifyagain" rel="guest">Notify Again</a>
							   ...
							   <div id="myalbum" class="tabDiv">
											...
											<?php foreach($myAlbum as $mAlbum){?>	
												<tr>
													...
													<td align="left" style="width:19%; border-bottom:dotted #CCCCCC 1px;">
														<?php
															$totalAlbum = array('useralbum_id' => $mAlbum['id']);
								...
								<div id="favoritelist" class="tabDiv">
									<h3>Favorite Lists</h3>
									<table cellpadding="5" cellspacing="0" style="table-layout:fixed; width:100%;">
										<!--<tr>
											<td>
												<?php if(isset($_SESSION['msg']) && $_SESSION['msg'] !=""){ ?>
													<div class="messSuccess" style="text-align: center;">
														<?php echo $_SESSION['msg']?><br />
														<a href="<?php echo $log->baseurl;?>index.php?pg=photocollection&id=<?php echo $_REQUEST['id'];?>">Click Here</a> to see the Favorite Lists.
													</div>
												<?php } ?>
											</td>
										</tr>-->
										<tr>
											<td>
												<?php
													$selectCond = array('user_email' => $_SESSION['email'],
																		 'album_id' => $_REQUEST['id'],
																		 'scode'  => $_SESSION['scode']);
																					
													$favoriteItems= $shDB->selectOnMultipleCondition($selectCond,'favourites');
													...
													if($favoriteItems){
														...
														foreach($favoriteItems as $favoriteItem){
															$data = array('id'=>$favoriteItem['img_id'],
																		  'album_id' => $favoriteItem['album_id']);
																		
															$outPut= $shDB->selectOnMultipleCondition($data,'album_image');
															$favoriteDatas[] = $outPut[0];
														}
														
														for($i=0;$i<sizeof($favoriteDatas);$i++){	
															echo '<div class="photoCollectionPhotoDiv" >';
															echo '<div style="height:125px;display:block;float:left;margin-left:0px;">
																	<a href="'.$useralbumurl.$favoriteDatas[$i]['image'].'" class="veryeasylightbox" rel="'.$useralbumurl.$favoriteDatas[$i]['image'].'" >
																		<div class="thumbimagediv"><img src="'.$useralbumthumburl.$favoriteDatas[$i]['image'].'" class="photoThumb" /></div>
																	</a>
																 </div>
																 </div>';
														}
													}else{
														echo '<div style="float: left; width: 100%;text-align: center;"><strong> You have not added any photo as favorite yet.</strong></div>';
													}
												?>
											</td>
										</tr>
...
$order = $shDB->selectOnMultipleCondition($selectOrder,'`order`');
				...
				<table cellpadding="3" cellspacing="0" style="table-layout:fixed; width:100%;">
					...
					<tr>
						...
						<td align="left" style="border-bottom:dotted #CCCCCC 1px;"><?php echo $settings['currency'];?>&nbsp;<font class="currency"><?php echo number_format($ord['totalamount'],2,'.','');?></font></td>
