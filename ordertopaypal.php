...
if(isset($_REQUEST['randstring'])){
	...
	$datainsert = array(
		...
		if($result){
			$orderid = $dataInTempOrder[0]['id'];
			$orderamount = round($_REQUEST['totalprice'], 0);
		}
	}
}
?>
<h1 class="pagetitle">Order Payment</h1>
		...
		<table cellpadding="3" cellspacing="0" style="table-layout:fixed; width:100%;">
					<tr>
						<td align="left" class="" style="width:96%; padding-left:2%;">
							<h2 class="subTitleh2" style="width:808px;">Order Item and Invoice Details</h2>
						</td>
					</tr>
					<tr>
					<td>
...
switch ($_GET['action']) {
   ...
   case 'process':      // Process and order...
      ...
      $p->add_field('return', $log->baseurl.'index.php?pg=paypalreturn&action=success&userid='.$_SESSION['userid'].'&orderid='.$orderid);
      $p->add_field('cancel_return', $log->baseurl.'index.php?pg=paypalreturn&action=cancel&userid='.$_SESSION['userid'].'&orderid='.$orderid);
      $p->add_field('notify_url', $log->baseurl.'index.php?pg=paypalreturn&action=ipn&userid='.$_SESSION['userid'].'&orderid='.$orderid);
      $p->add_field('item_name', 'Album Order');
       $p->add_field('currency', 'AUD');
